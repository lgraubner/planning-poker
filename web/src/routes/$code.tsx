    ...
    <CenteredSection>
      ...
      >
        ...
        <form.Subscribe selector={(state) => state.values.name}>
          {(name) => <Button disabled={!name.trim()}>Join room</Button>}
        </form.Subscribe>
...
function Room({ code, participant }: { code: string; participant: Identity }) {
  ...
  return (
    <section className="flex grow flex-col">
      <RoomHeader title={snapshot.title} copyMessage={copyMessage} onCopy={copyLink} />
      {!connected && (
      ...
      {snapshot.revealed ? (
        <section
          ...
          className={`${controls} flex min-h-28 items-center justify-center border-t border-zinc-700`}
        >
        ...
        <EstimateControls connected={connected} estimate={own?.estimate} send={send} />
      )}
    </section>
  );
}
...
function RoomHeader({
  ...
  return (
    <header className="mb-2 flex min-h-11 items-start justify-between gap-3">
      <div className="min-w-0">
        <p className="mb-1 text-sm leading-tight font-medium text-zinc-400">Planning Poker</p>
        <h1 title={title} className="truncate text-lg leading-tight font-semibold tracking-tight">
          {title}
        </h1>
      </div>
      <div className="relative mt-0.5 shrink-0">
        <Button compact variant="ghost" onClick={onCopy}>
          Copy room link
        </Button>
        <span
          role="status"
...
function ParticipantCard({
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className={`mx-auto flex h-18 w-12 items-center justify-center rounded-lg border-2 text-xl font-semibold text-indigo-400 transition-colors duration-150 motion-reduce:transition-none ${
...
function EstimateControls({
  ...
  return (
    <section aria-label="Estimate controls" className={controls}>
      <div
        role="group"
...
const controls =
  'sticky bottom-0 z-10 w-full max-w-7xl self-center bg-background pt-3.5 pb-2.5 text-center';
